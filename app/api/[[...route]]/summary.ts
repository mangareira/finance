...
const app = new Hono().get(
  ...
  async (c) => {
    ...
    const { from, accountId, to } = c.req.valid('query');
    ...
    const peridoLength = differenceInDays(endDate, startDate) + 1;
    const lastPeriodStart = subDays(startDate, peridoLength);
    const lastPeriodEnd = subDays(endDate, peridoLength);
